...
private int numberOfEntries;
    ...
    /*
    public class LinearProbingWithCount<K, V>
    {
...
public int search(K key) {
        ...
        //Starts probing for an empty place
        while (hashTable[index] != null) {
            probes++;
            if (hashTable[index].getKey().equals(key)) {
                return probes; // found the key
            }
            index = (index + 1) % hashTable.length; //linear probing alg
        }
    ...
    public int LinearProbe(int index, K key){
        ...
        while (!found && hashTable[index] != null) {
            ...
            if (hashTable[index] != AVAILABLE) {
                ...
                if (avalibleStateIndex == -1)
                    avalibleStateIndex = index;
                index = (index + 1) % hashTable.length;
            }

        }
       /*
        if (found || (avalibleStateIndex == -1)) {
            avalibleStateIndex = index;
            //return index;
        } else {

            //return avalibleStateIndex;
        }
        */
        probeCount += compCount;
        return compCount;
